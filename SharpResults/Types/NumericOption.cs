...
[SuppressMessage("Design", "CA1000:Do not declare static members on generic types",
    Justification = "Required for INumber.")]
[Serializable]
[JsonConverter(typeof(NumericOptionJsonConverter))]
public readonly struct NumericOption<T> : IEquatable<NumericOption<T>>, IComparable<NumericOption<T>>,
    ...
    where T : struct, INumber<T>
{
    ...
    /// </summary>
    [SuppressMessage("Design", "CA1000:Do not declare static members on generic types",
        Justification = "The syntax `NumericOption<T>.None` is too nice to give up.")]
        ...
        IFormatProvider? provider)
    {
        if (_isSome)
        {
            if ("Some(".TryCopyTo(destination) &&
                _value.TryFormat(destination[5..], out var innerWritten, format, provider))
            {
        ...
        IFormatProvider? provider)
    {
        if (_isSome)
        {
            ...
            {
                if ("Some("u8.TryCopyTo(utf8Destination) &&
                    spanFormattable.TryFormat(utf8Destination[5..], out int innerWritten, format, provider))
                {
    ...
    public static NumericOption<T> Clamp(NumericOption<T> value, NumericOption<T> min, NumericOption<T> max)
    {
        return value._isSome && min._isSome && max._isSome
            ? new(T.Clamp(value._value, min._value, max._value))
            : default;
    }
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    static bool INumberBase<NumericOption<T>>.TryConvertFromSaturating<TOther>(TOther value,
        out NumericOption<T> result)
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    static bool INumberBase<NumericOption<T>>.TryConvertFromTruncating<TOther>(TOther value,
        out NumericOption<T> result)
    ...
    public static implicit operator NumericOption<T>(Option<T> option)
        => option.WhenSome(out var val) ? new(val) : default;
