...
public static class OptionResultExtensions
{
        ...
        where TErr : notnull
    {
        ...
        {
            return result.Match(
                ok: val => Result.Ok<Option<T>, TErr>(Option.Some(val)),
                err: Result.Err<Option<T>, TErr>
            );
        }
    ...
    {
        return self.Match(
            ok: x => x.WhenSome(out var value) ? Option.Some(Result.Ok<T, TErr>(value)) : default,
            err: e => Option.Some(Result.Err<T, TErr>(e))
        );
    }
}
